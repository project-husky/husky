...
public class RecommendedMedNarrativeTextGenerator {
	 ...
	 */
	public RecommendedMedNarrativeTextGenerator(List<POCDMT000040Entry> entries,
	...
	private StrucDocTable getBodyPrescriptionEntry(POCDMT000040SubstanceAdministration substance, int index) {
		...
		if (substance != null) {
			...
			if (!substance.getId().isEmpty() && substance.getId().get(0) != null) {
				addRow("VerordnungsID", substance.getId().get(0).getRoot() + " / "
	...
	private String[] convertDateToGermanWeekDay(String[] weekDays, Date date) {
