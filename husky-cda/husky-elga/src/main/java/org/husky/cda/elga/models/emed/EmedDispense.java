...
import org.husky.cda.elga.generated.artdecor.emed.AbgabeSection;
import org.husky.cda.elga.generated.artdecor.emed.EMedikationAbgabe;
...
import org.husky.cda.elga.narrative.DispenseNarrativeTextGenerator;
...
public class EmedDispense  {
	...
	protected POCDMT000040DocumentationOf getAtcdabbrHeaderDocumentationOfServiceEvent() {
		...
		if(kindOfPrescription != null) {
			...
			if (kindOfPrescription.getCode().equalsIgnoreCase(ElgaMedikationRezeptart.KASSENREZEPT_CODE)
					|| kindOfPrescription.getCode().equalsIgnoreCase(ElgaMedikationRezeptart.PRIVATREZEPT_CODE)) {
			...
			if (startValidityPeriod != null && stopValidityPeriod != null) {
				...
				documentationOfServiceEvent.getServiceEvent().setEffectiveTime(time);
			} else {
				IVLTS ivlts = new IVLTS();
				ivlts.nullFlavor = new ArrayList<>();
				ivlts.nullFlavor.add(NullFlavor.MASKED_CODE);
				documentationOfServiceEvent.getServiceEvent().setEffectiveTime(ivlts);
			}
		}
	...
	protected POCDMT000040StructuredBody getHl7CdaR2Pocdmt000040StructuredBodyEMedicationPrescription() {
		...
		if (this.prescriptionEntries != null && !this.prescriptionEntries.isEmpty()) {
			...
			comp3.setSection(getAbgabeSection());
	...
	protected AbgabeSection getAbgabeSection() {
		if (this.prescriptionEntries == null || this.prescriptionEntries.isEmpty()) {
			return null;
		}

		AbgabeSection deliverySection = new AbgabeSection();

		int index = 0;
		for (PrescriptionEntry entry : prescriptionEntries) {
			if (entry != null) {
				deliverySection.getEntry().add(entry.getMedikationVerordnungEntryemedEmedication(index, gdaId, kindOfPrescription));
			}
		}
		...
		DispenseNarrativeTextGenerator textbuilder = new DispenseNarrativeTextGenerator(deliverySection.getEntry(),
				patientInstructions, pharmacistInstructions);
		textKonsUeberwGrund.getContent().add(textbuilder.toString());
		deliverySection.setText(textKonsUeberwGrund);

		return deliverySection;
	}
	
	public EMedikationAbgabe getEMedicationDelivery() {
		EMedikationAbgabe cda = new EMedikationAbgabe();
	...
	private void addHeader(EMedikationAbgabe cda) {
