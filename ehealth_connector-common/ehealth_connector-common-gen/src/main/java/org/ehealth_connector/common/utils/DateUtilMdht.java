...
public class DateUtilMdht {
			...
			if (effectiveTime instanceof IVL_TS) {
				...
				if (effectiveTimeInterval.getLow() != null) {
					...
					if ((effectiveTimeInterval.getHigh() != null)
						...
						final String tsHighStr = DateUtilMdht.formatDateCH(tsHigh);
						final String tsLowStr = DateUtilMdht.formatDateCH(tsLow);
				...
				if (effectiveTime instanceof TS) {
					...
					final String tsStr = DateUtilMdht.formatDateCH(ts);
					b.append(tsStr);
				}
				b.append(" ");
			}
	...
	public static IVXB_TS createIVXB_TSFromDate(Date date) {
		...
		if (date == null) {
			...
			ohtIntervalPoint.setValue(DateUtilMdht.formatDate(date));
		}
	...
	public static IVXB_TS createIVXB_TSFromDateTime(Date date) {
		...
		if (date == null) {
			...
			ohtIntervalPoint.setValue(DateUtilMdht.format(date));
		}
		return ohtIntervalPoint;
	}
	 ...
	 * @return the sxcm ts
	 */
	public static SXCM_TS createSTCM_TS(Date startOfPossibleAppliance,
	 ...
	 *
	 * @param eurDateStr
	 *            <div class="en">a String in the european format
	 *            dd.MM.yyyy</div> <div class="de">Ein String in dem Format
	 *            dd.MM.yyyy wie er in Europa gebräuchlich ist</div>
	...
	/**
	 * Check whether the dates of the two dates are equal (ignoring the time of
	 * the day).
	 *
	 * @param validFrom
	 *            the valid from
	...
	/**
	 * <div class="en">Date in dd.MM.yyyy HH:mm:ss</div>
	 *
	 * @param dateSt
	 *            <br>
	 *            <div class="en"> date String</div>
	 ...
	 */
	public static LocalDateTime localDateTime(String dateSt) {
		DateTimeFormatter df = DateTimeFormatter.ofPattern("dd.MM.yyyy HH:mm:ss");
		return LocalDateTime.parse(dateSt, df);
	}
	...
	public static Date parseDates(String value) {
		final String[] formatStrings = { "yyyyMMddHHmmssZZZZ", "yyyyMMddHHmmZZZZ", "yyyyMMddHHmm",
	...
	public static TS ts(Date date) {
		...
		timestamp.setValue(DateUtilMdht.formatDate(date));
		return timestamp;
	}
