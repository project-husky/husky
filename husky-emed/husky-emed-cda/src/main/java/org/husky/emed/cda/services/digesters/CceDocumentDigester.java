...
import org.husky.emed.cda.services.readers.NameReader;
...
public class CceDocumentDigester {
    ...
    public static PatientDigest getPatientDigest(final POCDMT000040ClinicalDocument cce) {
        ...
        final var digest = new PatientDigest();

        patientRole.getId().stream().map(IiUtils::getNormalizedCx).forEach(digest.getIds()::add);
        Optional.ofNullable(patient.getAdministrativeGenderCode()).map(CD::getCode).map(AdministrativeGender::getEnum).ifPresent(digest::setGender);
        Optional.ofNullable(patient.getBirthTime()).map(DateTimes::toLocalDate).ifPresent(digest::setBirthDate);

        final var names = new NameReader(patient.getName());
        digest.setFamilyName(names.getFirstLegalFamilyName());
        digest.setGivenName(names.getFirstLegalGivenName());
