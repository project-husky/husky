...
public class CDAR2Extractor {
    ...
    /**
     * Map between CDA R2 Administrative Gender codes and HL7v2.5 Table 001(Administrative Sex). Keys are CDAR2
     * AdministrativeGender object literal string representations and values are corresponding HL7v2.5 gender code
     * values as strings.
     */
     ...
     * Loads CDA Document
     *
     * @param cda the CDA R2 document to extract metadata from for the document entry metadata. Cannot be null.
     */
    ...
    /**
     * Entry point to invoke extraction process. <br> NOTES:
     * <ul>
...
/////////////////////////////////////////////////////////////////////////////
    ...
    /**
     * Extracts the relevant authorInstitution, authorPerson, authorRole and authorSpeciality information from the
     * author list in the CDA: <br> ClinicalDocument/author/assignedAuthor/id and <br>
    ...
    /**
     * Extracts the relevant classCode information from the CDA: ClinicalDocument/code. Implementation consistent with
     * PCC TF-2 Medical Document Binding to XDS, XDM and XDR. <br> Note: typeCode and classCode data sources are the
     * same in the CDA
     */
     ...
     * Implementation consistent with PCC TF-2 Medical Document Binding to XDS, XDM and XDR.
     */
    ...
    /**
     * Extracts the relevant creationTime information from the CDA: ClinicalDocument/effectiveTime. This time is to be
     * in UTC, but without the timezone offset or fractional seconds: [[[[[YYYY]MM]DD]HH]mm]ss]. Thus, this time is
     * assumed to be in GMT. So, if the timestamp in the corresponding CDA element contains the timezone offset, this
     * method will convert the timestamp to GMT. If the timezone offset is ommitted, then the timestamp is assumed to be
     * in GMT. Implementation consistent with PCC TF-2 Medical Document Binding to XDS, XDM and XDR.
     */
    ...
    /**
     * DocumentEntry.entryUUID is an XDS specific attribute and outside the scope of any CDA R2 document. Thus, it
     * cannot be extracted.
     *
     * @return null
     */
    ...
    /**
     * This extraction routine, at this point, does not attempt to extract event code metadata from a generic CDA R2
     * document.
     *
     * @return null
     */
    ...
    /**
     * This extraction routine, at this point, does not attempt to extract extension metadata from a generic CDA R2
     * document.
     *
     * @return null
     */
    ...
    /**
     * DocumentEntry.formatCode is an XDS specific attribute and outside the scope of any CDA R2 document. Thus, it
     * cannot be extracted.
     *
     * @return null
     */
    ...
    /**
     * DocumentEntry.hash is a comptuted value and outside the scope of any CDA R2 document. Thus, it cannot be
     * extracted.
     *
     * @return null
     */
     ...
     * Per the CDA specification this code, when present shall be consistent with ClinicalDocument/code. Thus,
     * implementation consistent with PCC TF-2 Medical Document Binding to XDS, XDM and XDR. <br> Note:
     * healthcareFacilityTypeCode and practiceSettingCode data sources are the same in the CDA
     */
...
// * to XDS, XDM and XDR.
    ...
    /**
     * Extracts the relevant legalAuthenticator information from the CDA: ClinicalDocument/legalAuthenticator/assignedEntity/id
     * and ClinicalDocument/legalAuthenticator/assignedEntity/assignedPerson/name <br> NOTES:
     * <ul>
    ...
    /**
     * DocumentEntry.mimeType is an XDS specific attribute and outside the scope of any CDA R2 document. Thus, it cannot
     * be extracted.
     *
     * @return null
     */
    ...
    /**
     * DocumentEntry.parentDocument expresses the id and relationship of a parent document to this CDA document in the
     * XDS Regisry. This is not necessairly the same parent document that is documented within the CDA. Thus, this
    ...
    /**
     * DocumentEntry.patientId expresses the Affinity Domain level patient id used in the XDS Regisry. In all cases,
     * this is not necessairly the same patientId that is documented within the CDA. Thus, this information is not
     * extracted.
     *
     * @return null
     */
    ...
    /**
     * DocumentEntry.practiceSettingCode is recommended to be supplied by the document source from an established
     * vocabulary approved by the affinity domain, such as that described by the Subject Matter Domain in LOINC. Thus
     * this information is not extracted. <br> Note: Implementation consistent with PCC TF-2 Medical Document Binding to
     * XDS, XDM and XDR.
     */
    ...
    /**
     * DocumentEntry.repositoryUniqueId is an XDS specific attribute and outside the scope of any CDA R2 document. Thus,
    ...
    /**
     * Extracts the relevant serviceStartTime information from the CDA: ClinicalDocument/documentationOf/serviceEvent/effectiveTime
     * This time is to be in UTC, but without the timezone offset or fractional seconds: [[[[[YYYY]MM]DD]HH]mm]ss].
     * Thus, this time is assumed to be in GMT. So, if the timestamp in the corresponding CDA element contains the
     * timezone offset, this method will convert the timestamp to GMT. If the timezone offset is ommitted, then the
     * timestamp is assumed to be in GMT. NOTES:
     * <ul>
    ...
    /**
     * Extracts the relevant serviceStopTime information from the CDA: ClinicalDocument/documentationOf/serviceEvent/effectiveTime
     * This time is to be in UTC, but without the timezone offset or fractional seconds: [[[[[YYYY]MM]DD]HH]mm]ss].
     * Thus, this time is assumed to be in GMT. So, if the timestamp in the corresponding CDA element contains the
     * timezone offset, this method will convert the timestamp to GMT. If the timezone offset is ommitted, then the
     * timestamp is assumed to be in GMT. <br> NOTES:
     * <ul>
    ...
    /**
     * DocumentEntry.size is a comptuted value and outside the scope of any CDA R2 document. Thus, it cannot be
     * extracted.
     *
     * @return null
     */
    ...
    /**
     * Extracts the relevant sourcePatientId information from the CDA: ClinicalDocument/recordTarget/patientRole/id <br>
     * NOTES:
     * <ul>
    ...
    /**
     * Extracts the relevant sourcePatientInfo information from the CDA: ClinicalDocument/recordTarget/patientRole <br>
     * NOTES:
     * <ul>
     * <li>1. Only the first ClinicalDocument/recordTarget instance will be
     * considered for sourcePatientId (for which only the first patientRole/id on
    ...
    /**
     * Extracts the relevant title, Implementation consistent with PCC TF-2 Medical Document Binding to XDS, XDM and
     * XDR. information from the CDA: ClinicalDocument/title
     */
    ...
    /**
     * Extracts the relevant typeCode. Implementation consistent with PCC TF-2 Medical Document Binding to XDS, XDM and
     * XDR. information from the CDA: ClinicalDocument/code <br> Note: typeCode and classCode data sources are the same
     * in the CDA
     */
    ...
    /**
     * Extracts the relevant uniqueId. Implementation consistent with PCC TF-2 Medical Document Binding to XDS, XDM and
     * XDR. information from the CDA: ClinicalDocument/id (format is id@root^id@extension)
     */
    ...
    /**
     * DocumentEntry.URI is a comptuted value and outside the scope of any CDA R2 document. Thus, it cannot be
     * extracted.
     *
     * @return null
     */
...
/////////////////////////////////////////////////////////////////////////////
     ...
     *
     * @param tel ClinicalDocument/author/assignedAuthor/representedOrganization of type Organization expected.
...
/////////////////////////////////////////////////////////////////////////////
     ...
     *
     * @param idList ClinicalDocument/recordTarget/patient/id(s) list of type II expected
     ...
     *
     * @param nameList ClinicalDocument / recordTarget / patient / patientPatient / name(s) list of type PN expected.
     ...
     *
     * @param time ClinicalDocument / recordTarget / patient / patientPatient / birthTime of type TS1 expected
     * @return
     */
     ...
     *
     * @param docCode ClinicalDocument / recordTarget / patient / patientPatient / administrativeGenderCode of type CE
     *                expected
     * @return
     */
    ...
    /**
     * Maps CDA R2 II and PN datatypes to the OHT model version of the HL7 v2.5 XCN data type. Implementation consistent
     * with PCC TF-2 Medical Document Binding to XDS, XDM and XDR.
     *
     * @param id
     * @param name
     * @return
    ...
    /**
     * Maps CDA R2 II and ON datatypes to the OHT model version of the HL7 v2.5 XON data type. Implementation consistent
     * with PCC TF-2 Medical Document Binding to XDS, XDM and XDR, pending resolution to ITI CP 316.
     *
     * @param id
     * @param name
     * @return
    ...
    /**
     * Maps CDA R2 TS1 data type to the OHT model version of the Hl7 v2.5 TS data type. The XDS Metadata timestamp is to
     * be in UTC, but without the timezone offset or fractional seconds: [[[[[YYYY]MM]DD]HH]mm]ss]. Thus, this time is
     * assumed to be in GMT. So, if the timestamp in the corresponding CDA element contains the timezone offset, this
     * method will convert the timestamp to GMT. If the timezone offset is ommitted, then the timestamp is assumed to be
     * in GMT. <br>
     *
