...
public class IndexDbAugmentationService {
     ...
     */
    public void augment(final NarrativeTreatmentItem item,
                        final NarrativeLanguage lang) {
        if (item.getCodeType() == ProductCodeType.GTIN) {
            final var gtin = item.getProductCode();

            if (item.getProductIngredients().size() == 0) {
                final var activeIngredients = this.getActiveIngredients(gtin);
     ...
     */
    private List<NarrativeTreatmentIngredient> getActiveIngredients(final String gtin) {
        final var sqlSelectActiveIngredient = """
        SELECT ms.name_fr, mcs.qty, mcs.qty_unit
        FROM med_article AS mas
            INNER JOIN med_barcode AS mb
                ON mas.pharmacode = mb.pharmacode
            INNER JOIN med_product AS mp
                ON mas.product_no = mp.product_no
        ...
        try(final Connection conn = this.getConnection()) {
            final PreparedStatement ps = conn.prepareStatement(sqlSelectActiveIngredient);
            ps.setString(1, gtin);

            final ResultSet rs = ps.executeQuery();
            final var activeIngredients = new ArrayList<NarrativeTreatmentIngredient>();
            while (rs.next()) {
                final String substanceName = rs.getString("name_fr");
                final String substanceQty = rs.getString("qty");
                final String substanceUnit = rs.getString("qty_unit");

                final var quantity = new ChEmedQuantityWithEmedUnits()
                        .setQuantity(new BigDecimal(substanceQty))
                        .setUnitCode(UnitCode.getEnum(substanceUnit));

                activeIngredients.add(new NarrativeTreatmentIngredient(substanceName, quantity));
            }
