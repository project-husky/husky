...
import org.projecthusky.fhir.emed.ch.epr.model.emediplan.chmed23a.ChMed23APatient;
import org.projecthusky.fhir.emed.ch.epr.model.emediplan.chmed23a.ChMed23APatientId;
import org.projecthusky.fhir.emed.ch.epr.model.emediplan.chmed23a.ChMed23APostalAddress;
...
public class ChMed23AConverterTest {
    ...
    void convertAndValidateMedicationCard() throws IOException {
        ...
        final var converter = new ChMed23AConverter();
    ...
    /**
     * This test tries to verify the validation of all the {@link ChMed23APatient} fields, except the extensions and
    ...
    void testPatientValidation() {
        ...
        patient.setAddress(new ChMed23APostalAddress(null, null, null, "55"));
        result = patient.validate();
        assertTrue(result.isSuccessful());
        assertEquals(1, result.getWarnings().size());

        // set address to more than 2 characteres, expect one warning
        patient.setAddress(new ChMed23APostalAddress(null, null, null, "ABC"));
        result = patient.validate();
        assertTrue(result.isSuccessful());
        assertEquals(1, result.getWarnings().size());
        ...
        patient.setIds(List.of(new ChMed23APatientId(null, "fakesystem", "fakeid")));
        result = patient.validate();
        assertFalse(result.isSuccessful());

        // set id of type insurance card number with a system, expect warning
        patient.setIds(List.of(new ChMed23APatientId(PatientIdType.INSURANCE_CARD_NUMBER, "fakesystem", "fakeid")));
        result = patient.validate();
        assertTrue(result.isSuccessful());
        assertEquals(1, result.getWarnings().size());

        // set id of type insurance card number without a system and without a value, exepct failure
        patient.setIds(List.of(new ChMed23APatientId(PatientIdType.INSURANCE_CARD_NUMBER, "", "")));
        result = patient.validate();
        assertFalse(result.isSuccessful());

        // set id of local type with no system, expect failure
        patient.setIds(List.of(new ChMed23APatientId(PatientIdType.LOCAL_PID, "", "fakeid")));
        result = patient.validate();
        assertFalse(result.isSuccessful());

        // set id of local type with system but not value, expect failure
        patient.setIds(List.of(new ChMed23APatientId(PatientIdType.LOCAL_PID, "myfakesystem", "")));
        result = patient.validate();
        assertFalse(result.isSuccessful());
    ...
    private ChMed23APatient createValidPatient() {
        return new ChMed23APatient(
                "John",
                "Doe",
                LocalDate.of(1958, 2, 3),
                ChMed23AGender.MALE,
                new ChMed23APostalAddress("Elm Street", "1203", "Springfield", "US"),
                "en",
                List.of(new ChMed23APatientId(PatientIdType.LOCAL_PID, "http://myfakesystem", "myfakeid")),
                null,
                null,
                List.of(),
