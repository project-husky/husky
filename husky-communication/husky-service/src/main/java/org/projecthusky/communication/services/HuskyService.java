...
/**
 * The main service class to use when working with the EPD web services. As it
 * is marked as a @Component, it will be immediately available in your
 * application to reference.<br/>
 * It provides query building capabilities and executing those queries against a
 * targeted service.<br/>
 ...
 * <li>Searching for a Patient's documents -
 * {@link XdsRegistryStoredFindDocumentsQuery}</li>
 * </ul>
 * Use it as follows:
 * 
 * <pre class="code">
...
public class HuskyService {
	...
	public HuskyService(HuskyWebServiceClient wsClient) { // leverage
															// constructor
															// injection.
	...
	/**
	 * This method will start building a {@link XdsDocumentSetRequest}, which allows
	 * the user to download the documents ofa Patient.
	 *
	 ...
	 * {@link XdsRegistryStoredFindDocumentsQuery}, which allows the user to look
	 * for documents of a Patient identified by the ID by various parameters.
	 *
	 * @return RegistryStoredFindDocumentsQueryBuilder
	 */
	public XdsRegistryStoredFindDocumentsQuery.XdsRegistryStoredFindDocumentsQueryBuilder<?, ?> createFindDocumentsStoredQuery(
			Destination destination, Identificator patientID, SecurityHeaderElement xuaToken) {
		return XdsRegistryStoredFindDocumentsQuery.builder().destination(destination).patientId(patientID)
				.xuaToken(xuaToken);
	}

	/**
	 * Method to create a GetSubmissionSets query builder.
	 * 
	 * @param destination {@link Destination}
	 * @return builder that allows to build the query
	 */
	public XdsRegistryStoredGetSubmissionSetsQuery.XdsRegistryStoredGetSubmissionSetsQueryBuilder<?, ?> createGetSubmissionSetsQuery(
			Destination destination, SecurityHeaderElement xuaToken) {
		return XdsRegistryStoredGetSubmissionSetsQuery.builder().destination(destination).xuaToken(xuaToken);
	}

	/**
	 * Method to create a GetDocumentsAndAssociations query builder.
	 * 
	 * @param destination {@link Destination}
	 * @param xuaToken
	 * @return builder that allows to build the query
	 */
	public XdsRegistryStoredGetDocumentsAndAssociationsQuery.XdsRegistryStoredGetDocumentsAndAssociationsQueryBuilder<?, ?> createGetDocumentsAndAssociationsQuery(
			Destination destination, SecurityHeaderElement xuaToken) {
		return XdsRegistryStoredGetDocumentsAndAssociationsQuery.builder().destination(destination).xuaToken(xuaToken);
	}

	/**
	 * This method will start building a
	 * {@link XdsRegistryStoredFindSubmissionSetsQuery}, which allows the user to
	 * look for submission sets of a Patient identified by the ID by various
	 * parameters.
	 *
	 * @return FindSubmissionSetsStoredQueryBuilder
	 */
	public XdsRegistryStoredFindSubmissionSetsQueryBuilder<?, ?> createFindSubmissionSetsStoredQuery(
			Destination destination, Identificator patientId, SecurityHeaderElement xuaToken) {
		return XdsRegistryStoredFindSubmissionSetsQuery.builder().destination(destination).patientId(patientId)
				.xuaToken(xuaToken);
	}

	/**
	 * Method to create a GetAssociations query builder.
	 * 
	 * @param destination {@link Destination}
	 * @param xuaToken    the XUA token
	 * @return builder that allows to build the query
	 */
	public XdsRegistryStoredGetAssociationsQueryBuilder<?, ?> createGetAssociationsQuery(Destination destination,
			SecurityHeaderElement xuaToken) {
		return XdsRegistryStoredGetAssociationsQuery.builder().destination(destination).xuaToken(xuaToken);
	}

	/**
	 * This method will start building a {@link XdsFindFoldersStoredQuery}, which
	 * allows the user to search the folders of a Patient.
	 */
	...
	/**
	 * This method will start building a {@link XdsRegistryStoredGetAllQuery}, which
	 * allows the user to retrieve all documents, submission sets, folders, and
	 * associations known for a patient.
	 *
	 * @return GetAllStoredQueryBuilder
	 */
	public XdsRegistryStoredGetAllQuery.XdsRegistryStoredGetAllQueryBuilder<?, ?> createGetAllStoredQuery(
			Destination destination, SecurityHeaderElement xuaToken) {
		return XdsRegistryStoredGetAllQuery.builder().destination(destination).xuaToken(xuaToken);
	}

	/**
	 * This method will start building a {@link HpdAddFeed}, which is needed to
	 * create the HPD request
	 */
	 ...
	 * multiple HPD requests. The request also provides more options like how to
	 * process the request (sequential or in parallel), how to retrieve the response
	 * (sequential or unordered) and how to proceed in case of an error (resume or
	 * exit).
	 */
	...
	/**
	 * This method will start building a {@link HpdDeleteFeed}, which is needed to
	 * create the HPD request
	 */
	...
	/**
	 * This method will start building a {@link HpdSearchQuery}, which is needed to
	 * create the HPD request
	 */
	...
	/**
	 * Use this method to start building a {@link PdqSearchQuery}. These queries are
	 * designed to search for patients by different criteria.<br/>
	 * Previously they where called Patient Demographics Query (PDQ), as the
	 * official definition states.<br/>
	 * Common criteria are:
	 * <ul>
	 ...
	 *
	 * @return A SearchQueryBuilder to build a {@link PdqSearchQuery} with the
	 *         parameters you wish.
	 * @see <a href=
	 *      "https://www.e-health-suisse.ch/technik-semantik/epd-anbindung/informatikverantwortliche.html">Patient
	 *      suchen</a>
	 */
	...
	/**
	 * This method will start building a {@link PixAddPatientFeed}, which allows the
	 * user to create a new Patient in the system.<br/>
	 * The properties of an {@link FhirPatient} are used as a template and expected
	 * to be filled.
	 *
	 * @param destination {@link Destination}
	 * @return PixAddPatientQueryBuilder - to build the query
	 * @see <a href=
	 *      "https://www.e-health-suisse.ch/technik-semantik/epd-anbindung/informatikverantwortliche.html">Registry
	 *      Stored Query</a>
	 */
	...
	/**
	 * Use this method to build a {@link PixMergePatientFeed}, with which the user
	 * can merge two know patient objects into one in case those two are the same
	 * person.<br/>
	 * This case is mostly used to correct user errors when the same person is
	 * entered into a system multiple times and needs to be merged into one.
	 *
	 * @param destination            {@link Destination}
	 * @param providerOrganization   {@link Organization}
	 * @param obsoletePatientId      - the ID of the patient that should be merged
	 *                               into the new one.
	...
	public PixMergePatientFeed.PixMergePatientFeedBuilder createPixMergePatientFeed(Destination destination,
			Organization providerOrganization, String obsoletePatientId, String homeCommunityOID,
			String homeCommunityNamespace) {
	...
	/**
	 * Use this method to build a {@link PixPatientIDQuery}, where the user can ask
	 * for all IDs known of a patient.
	 */
	...
	/**
	 * This method will start building a {@link PixUpdatePatientFeed}, which allows
	 * the user to update an existing patient in the system.<br/>
	 * The properties of an {@link FhirPatient} are used, fill them accordingly.
	 *
	 * @param destination          {@link Destination}
	 * @param providerOrganization {@link Organization}
	...
	/**
	 * This method will start building a {@link SvsValueSetRequest}, which allows to
	...
	/**
	 * This method will start building a {@link XuaRequest}, which will be sent to
	 * the webservice and returns details about the identity of the authenticated
	 * user
	 */
	...
	/**
	 * Creates {@link org.openehealth.ipf.commons.ihe.hpd.stub.dsmlv2.SearchRequest}
	 * which queries the Healthcare Provider Directory returning a
	 * {@link BatchResponse} containing informations regarding the: Individual
	 * Provider Organizational Provider Relationship
	 *
	 * Creates {@link org.openehealth.ipf.commons.ihe.hpd.stub.dsmlv2.AddRequest}
	 * which writes to the Healthcare Provider Directory a: Individual Provider,
	 * Organizational Provider or a Relationship returning a {@link BatchResponse}
	 * containing a response code for success and error
	 *
	 * Creates {@link org.openehealth.ipf.commons.ihe.hpd.stub.dsmlv2.DelRequest}
	 * which deletes from the Healthcare Provider Directory a: Individual Provider,
	 * Organizational Provider or a Relationship returning a {@link BatchResponse}
	 * containing a response code for success and error
	 *
	 * @param hpdBatchRequest contains a batch of
	 *                        {@link org.projecthusky.communication.requests.hpd.HpdRequest}
	 *
	...
	/**
	 * Sends the {@link HpdBatchRequest} to the HPD service and returns the raw
	 * {@link BatchResponse}
	 *
	 * @param batchRequest contains a batch of
	 *                     {@link org.projecthusky.communication.requests.hpd.HpdRequest}
	...
	public BatchResponse sendRaw(BatchRequest batchRequest, Destination destination) throws Exception {
		...
		return wsClient.sendHpdSearchRequest(batchRequest, destination);
	}

	/**
	 * Send your built {@link PdqSearchQuery} through the Internet and receive the
	 * results from the webservice.
	 *
	 * @param patientSearchQuery A {@link PdqSearchQuery} previously built.
	 * @return A {@link PdqSearchResults} object containing the matching
	 *         {@link FhirPatient} objects in a list.
	 ...
	 * @return True if the patient was added without error. Errors are reported as
	 *         exceptions.
	 * @throws Exception        errors occurring in the webservice client
	 * @throws RuntimeException For errors/exceptions happening in the webservice
	 *                          there will be a RuntimeException wrapped
	 ...
	 * @return True if the patient was updated without error. Errors are reported as
	 *         exceptions.
	 * @throws Exception        errors occurring in the webservice client
	 * @throws RuntimeException For errors/exceptions happening in the webservice
	 *                          there will be a RuntimeException wrapped
	 ...
	 * @param isUseRaw        flag that signals if the response is needed in raw
	 *                        dformat
	...
	/**
	 * Send your built {@link XdsDocumentSetRequest} through the Internet to the
	 * webservice and receive the documents of the Patient.
	 *
	 ...
	 *                          SOAPException thrown.
	 */
	public RetrievedDocumentSet send(XdsDocumentSetRequest retrievedDocumentsRequest) throws Exception {
	...
	/**
	 * Send your built {@link XdsFindFoldersStoredQuery} to the webservice to find
	 * folders (XDSFolder objects) in the registry for a given patientID with
	 * matching 'status' attribute
	 *
	...
	/**
	 * Send your built {@link XdsRegistryStoredFindDocumentsQuery} through the
	 * Internet to the webservice and receive the document handlers.
	 *
	 * @return {@link QueryResponse}
	 * @throws Exception        errors occurring in the webservice client
	 * @throws RuntimeException For errors/exceptions happening in the webservice
	...
	/**
	 * Use the {@link XuaRequest} which is sent to the webservice and returns
	 * details about the identity of the authenticated user
	 *
