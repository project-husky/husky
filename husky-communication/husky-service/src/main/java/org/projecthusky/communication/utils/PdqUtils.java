...
import org.projecthusky.communication.responses.pix.PixPatientIDEntry;
...
/**
 * A util class designed specifically to extract a list of {@link FhirPatient}s
 * from a {@link PdqSearchQueryResponse}.<br/>
 * Most code was copied over from {@link PdqV3Query} and
 * {@link PixPdqV3QueryBase}.
 *
 * @author szalai
 */
public class PdqUtils {
	...
	public static PixPatientIDEntry getPatientDomainIdEntry(PixV3QueryResponse v3PixConsumerResponse, String rootOid) {
		PixPatientIDEntry retVal = null;
		if ((rootOid != null) && (v3PixConsumerResponse != null) && ((!v3PixConsumerResponse.getPatientIds().isEmpty())
				|| (!v3PixConsumerResponse.getAsOtherIDs().isEmpty()))) {
			...
			for (var i = 0; i < v3PixConsumerResponse.getPatientIds().size(); i++) {
				final II id = v3PixConsumerResponse.getPatientIds().get(i);
				if (id != null && id.getRoot() != null && id.getRoot().equals(rootOid)) {
					retVal = PixPatientIDEntry.builder().id(id.getExtension()).oid(id.getRoot())
							.name(id.getAssigningAuthorityName()).build();
				}
			}
			if (retVal == null) {
				PixPatientIDEntry idFromOthers = getIdEntryFromOthers(v3PixConsumerResponse, rootOid);
				if (idFromOthers != null) {
					retVal = idFromOthers;
				}
			}
		}
	...
	private static PixPatientIDEntry getIdEntryFromOthers(PixV3QueryResponse v3PixConsumerResponse, String rootOid) {
		for (var i = 0; i < v3PixConsumerResponse.getAsOtherIDs().size(); i++) {
			final var idEntry = v3PixConsumerResponse.getAsOtherIDs().get(i);
			if ((idEntry.getId() != null) && (!idEntry.getId().isEmpty())) {
				final II id = idEntry.getId().get(0);
				if (id != null && id.getRoot() != null && id.getRoot().equals(rootOid)) {
					return PixPatientIDEntry.builder().id(id.getExtension()).oid(id.getRoot())
							.name(id.getAssigningAuthorityName()).build();
				}
			}
		}
