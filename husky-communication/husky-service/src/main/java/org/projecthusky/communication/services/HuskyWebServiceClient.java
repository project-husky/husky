...
/**
 * This class is implemented for the mid/low-level access of the EPD Services. <br/> For the actual transfer of messages
 * Apache Camel is used, which needs to be provided by the application binding this service. In case of a Spring (Boot)
...
public class HuskyWebServiceClient {
  ...
  private static final String HPD_ITI_58 = "hpd-iti58";
  private static final String HPD_ITI_59 = "hpd-iti59";
  private static final String PDQV3_ITI47 = "pdqv3-iti47";
  private static final String PIXV3_ITI44 = "pixv3-iti44";
  private static final String PIXV3_ITI45 = "pixv3-iti45";
  private static final String XDS_ITI18 = "xds-iti18";
  private static final String XDS_ITI43 = "xds-iti43";
  private static final String XDS_ITI41 = "xds-iti41";
  ...
  public Exchange send(String endpoint, Object body, SecurityHeaderElement securityHeaderElement, String messageId,
      Map<String, String> outgoingHttpHeaders) throws SerializeException, ParserConfigurationException, IOException {
  ...
  public PRPAIN201306UV02Type sendPdqSearchQueryRequest(PdqQuery request, SecurityHeaderElement security,
      URI pdqDest, String messageId)
      throws JAXBException, DataBindingException, ParserConfigurationException, SerializeException, IOException {
    String endpoint = createEndpoint(pdqDest.toString(), PDQV3_ITI47);
  ...
  public MCCIIN000002UV01Type sendPixUpdatePatientRequest(PRPAIN201302UV02Type request, SecurityHeaderElement assertion,
      URI pdqDest, String action, String messageId)
      throws JAXBException, SerializeException, ParserConfigurationException, IOException {
  ...
  public MCCIIN000002UV01Type sendITI44Query(PRPAIN201304UV02Type request, SecurityHeaderElement assertion, URI pdqDest,
  ...
  private String sendITI44Query(String request, SecurityHeaderElement assertion, URI pdqDest, String action,
    ...
    String endpoint = createEndpoint(pdqDest.toString(), PIXV3_ITI44);
  ...
  public PixV3QueryResponse sendQuery(PixV3QueryRequest request, SecurityHeaderElement assertion, URI pdqDest,
    ...
    var endpoint = createEndpoint(pdqDest.toString(), PIXV3_ITI45);
  ...
  public RetrievedDocumentSet send(XdsDocumentSetRequest request, SecurityHeaderElement securityHeader, URI destination,
    ...
    String endpoint = createEndpoint(destination.toString(), XDS_ITI43);
  ...
  /** It implements the following IHE transaction: [ITI-40] Provide Identity Assertions */
  public XuaResponse send(XuaRequest xuaRequest) throws ClientSendException {
    ...
    //TODO: Fix implementation of the AssertionBuilderImpl to retrieve the Assertion with the signature in place
    return new XuaResponse(xuaResponses.get(0).getAssertion());
  }

  /**
   * It implements the following IHE transaction: [ITI-41] Provide and Register Document Set – b
   * <p>
   * Using this method, both extraction modes can be set explicitly, e.g. if the default option is not suitable for all
   * kinds of request.
   */
  public Response sendProvideAndRegisterDocumentSetRequest(XdsProvideAndRetrieveDocumentSetQuery documentSet,
      SubmissionSetMetadataExtractionMode extractionMode, DocumentMetadataExtractionMode documentMetadataExtractionMode)
      throws SerializeException, ParserConfigurationException, IOException {
    ...
    if (documentSet.getAssociation() != null) {
      ...
      if (txnData.getDocuments() != null && !txnData.getDocuments().isEmpty()
        ...
        documentSet.getAssociation().setSourceUuid(txnData.getDocuments().get(0).getDocumentEntry().getEntryUuid());
    ...
    String endpoint = createEndpoint(documentSet.getDestination().getUri().toString(), XDS_ITI41);
  ...
  /** It implements the following IHE transaction: [ITI-48] Retrieve value set and value set raw */
  ...
  /**
   * It implements the following IHE transactions: [ITI-58] Provider Information Query - Search Request [ITI-59]
   * Provider Information Feed - Add Request [ITI-59] Provider Information Feed - Delete Request
   */
  public HpdResponse sendHpdBatchRequest(HpdBatchRequest hpdBatchRequest)
    ...
    String protocolPrefix = hpdBatchRequest.getHpdRequests().stream()
        .map(hpdRequest -> hpdRequest instanceof HpdSearchQuery ? HPD_ITI_58 : HPD_ITI_59)
    ...
    String endpoint = createEndpoint(hpdBatchRequest.getDestination().getUri().toString(), protocolPrefix);
  ...
  /** Method that can be used for sending an individual request e.g. {@link org.projecthusky.communication.requests.hpd.HpdAddFeed} */
  public HpdResponse sendHpdRequest(HpdRequest hpdRequest)
      throws SerializeException, ParserConfigurationException, IOException {
    HpdBatchRequest hpdBatchRequest = HpdBatchRequest.builder()
        .hpdRequest(hpdRequest)
        .build();
  ...
  private XuaClientConfig createXuaClientConfig(XuaRequest xuaRequest) {
    return new XuaClientConfigBuilderImpl()
        .clientKeyStore(xuaRequest.getClientKeyStore())
        .clientKeyStorePassword(xuaRequest.getClientKeyStorePass())
        .clientKeyStoreType(xuaRequest.getClientKeyStoreType())
        .url(xuaRequest.getRepositoryUri())
        .create();
  }
  ...
  private String createEndpoint(String destinationUri, String protocolPrefix) {
    boolean secure = destinationUri.contains(HTTPS_LITERAL);

    String endpoint = String.format(
        "%s://%s?inInterceptors=%s&inFaultInterceptors=%s&outInterceptors=%s&outFaultInterceptors=%s&secure=%s&audit=%s&auditContext=%s",
        protocolPrefix, destinationUri.replace(HTTPS_LITERAL, "").replace(HTTP_LITERAL, ""),
