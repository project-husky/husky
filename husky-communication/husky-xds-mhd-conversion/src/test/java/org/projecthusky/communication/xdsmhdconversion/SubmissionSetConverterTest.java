...
class SubmissionSetConverterTest {
    ...
    void testConvertSubmissionSet1() throws Exception {
        ...
        assertEquals("HCP",
                     ((PractitionerRole) list.getSource().getResource()).getCode().get(0).getCodingFirstRep().getCode());
    ...
    private void testListResource(final ListResource list,
        ...
        final Optional<Extension> sourceId = list.getExtension().stream()
                .filter(e -> "https://profiles.ihe.net/ITI/MHD/StructureDefinition/ihe-sourceId".equals(e.getUrl()))
                .findAny();

        final Optional<Extension> intendedRecipient = list.getExtension().stream()
                .filter(e -> "https://profiles.ihe.net/ITI/MHD/StructureDefinition/ihe-intendedRecipient".equals(
                        e.getUrl()))
                .findAny();

        final Optional<Extension> designationType = list.getExtension().stream()
                .filter(e -> "https://profiles.ihe.net/ITI/MHD/StructureDefinition/ihe-designationType".equals(
                        e.getUrl()))
                .findAny();
        ...
        if (limitedMetadata) {
            ...
            assertEquals("https://profiles.ihe.net/ITI/MHD/StructureDefinition/IHE.MHD.Comprehensive.SubmissionSet",
                         profile);
        }
        ...
        if (designationTypeVal != null) {
            ...
            assertEquals(designationTypeVal,
                         ((CodeableConcept) designationType.get().getValue()).getCoding().get(0).getCode());
        } else {
