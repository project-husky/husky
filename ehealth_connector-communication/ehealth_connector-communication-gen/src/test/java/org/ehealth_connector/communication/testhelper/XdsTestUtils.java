...
public class XdsTestUtils {
	...
	public static Code[] formatCodes = new Code[] { new Code("urn:ihe:rad:1.2.840.10008.5.1.4.1.1.88.59", "KOS Dokument") };

	public static Code[] confidentialityCodes = new Code[] { new Code("30001", "administrative Daten") };
	...
	public static ParentDocumentRelationshipType[] parentRelation = new ParentDocumentRelationshipType[] {
			ParentDocumentRelationshipType.APND_LITERAL, ParentDocumentRelationshipType.RPLC_LITERAL };
	...
	public XdsTestUtils() {
		...
		try {
			...
			eDateTimeRange1 = new org.ehealth_connector.communication.xd.storedquery.DateTimeRange(DateTimeRangeAttributes.CREATION_TIME,
					d1, d2);
			...
			LogFactory.getLog(getClass()).error("error initializing test data", e);
		}
	...
	public void log(StoredQueryParameterList aSqpl) {
		aSqpl.forEach(new Consumer<StoredQueryParameter>() {
			...
			public void accept(StoredQueryParameter t) {
				LogFactory.getLog(getClass()).debug(t.getName() + ": " + t.getValue());
			}
		});
	}
}
