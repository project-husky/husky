...
public class FhirCdaChVacd {
	...
	/**
	 * <div class="en">Person who performed the medication administration</div>
	 * <div class="de"></div><div class="fr"></div>
	 */
	...
	/**
	 * @return <div class="en">performer of the medication administration</div>
	 *         <div class="de">Person, welche das Medikament verabreicht
	 *         hat</div><div class="fr">personne qui a effectué l'administration
	 *         du médicament</div>
	 */
	...
	/**
	 * @param performer
	 *          <div class="en">performer of the medication administration</div>
	 *          <div class="de">Person, welche das Medikament verabreicht
	 *          hat</div><div class="fr">personne qui a effectué
	 ...
	 * @param bundle
	 *          the FHIR bundle
	 * @return eHC Author object containing the legal authenticator </div>
	 *         <div class="de"></div> <div class="fr"></div>
	 */
	 ...
	 * @param fhirMedicationStatement
	 *          the FHIR bundle
	 * @return eHC Author object containing the performer </div>
	 *         <div class="de"></div> <div class="fr"></div>
	 */
	...
	final org.ehealth_connector.cda.ch.enums.ProblemConcernStatusCode problemStatusCode = getProblemConcernStatusCode(
		fhirCondition);

	// Create the ActiveProblemConcern
	final org.ehealth_connector.cda.ActiveProblemConcern retVal = new org.ehealth_connector.cda.ActiveProblemConcern(
	...
	// Value
	retVal.addValue(new Code(FhirCommon.removeURIPrefix(fhirCode.getSystem()), fhirCode.getCode(),
		fhirCode.getDisplay()));
	...
	if (fhirMedicationStatement.getMedication() instanceof ResourceReferenceDt) {
		...
		final IdentifierDt id = (IdentifierDt) fhirMedi
			.getUndeclaredExtensionsByUrl(FhirCommon.urnUseAsIdentifier).get(0).getValue();
	...
	if (!"IMMUNIZ".equals(code.getCode())) {
		...
		entry.setImmunizationTargetCode(new Code(FhirCommon.removeURIPrefix(fhirCode.getSystem()),
			fhirCode.getCode(), fhirCode.getDisplay()));
	...
	final org.ehealth_connector.cda.LaboratoryObservation retVal = new LaboratoryObservation();

	final CodingDt fhirCode = fhirObservation.getCode().getCodingFirstRep();
	retVal.setCode(new Code(FhirCommon.removeURIPrefix(fhirCode.getSystem()), fhirCode.getCode(),
		fhirCode.getDisplay()));
	...
	if (fhirObservation.getValue() instanceof QuantityDt) {
		...
		retVal.addValue(
			new Value(fhirQuantity.getValue().toString(), Ucum.getEnum(fhirQuantity.getUnit())));
	...
	if (fhirInterpretationCode != null) {
		...
		retVal.setInterpretationCode(
			new Code(FhirCommon.removeURIPrefix(fhirInterpretationCode.getSystem()),
	...
	if (!fhirMedicationStatement.getUndeclaredExtensionsByUrl(FhirCommon.urnUseAsLotNumbertext)
		...
		final StringDt lot = (StringDt) fhirMedicationStatement
			.getUndeclaredExtensionsByUrl(FhirCommon.urnUseAsLotNumbertext).get(0).getValue();
		retVal = lot.toString();
	}
	return retVal;
	}
