...
public class FhirPatient extends ca.uhn.fhir.model.dstu2.resource.Patient {
	...
	if (getManagingOrganization() != null) {
		...
		organization.getIds()
			.add(new Identificator(oid, org.getIdentifierFirstRep().getValue()).getIi());
		}
	...
	if (getTelecom().size() > 0) {
		...
		if (ContactPointSystemEnum.PHONE
			...
			if (ContactPointUseEnum.WORK.equals(contactPointDt.getUseElement().getValueAsEnum())) {
	...
	final Address patientAddress = new Address(addressline1, addressline2, addressline3, zip, city,
		addressUse);
