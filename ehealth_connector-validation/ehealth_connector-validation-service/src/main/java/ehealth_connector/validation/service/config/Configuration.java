...
public class Configuration {
	 ...
	 */
	public Configuration(ConfigurationType configuration)
	...
	public String getDocumentSchema() throws ConfigurationException {
		...
		if (application != null) {
			...
			if (!schema.isFile()) {
				schema = new File(getBaseDir() + "/"
						+ application.getDocumentSchema());
			}
			if (!schema.isFile()) {
			...
			if (!schema.canRead()) {
				throw new ConfigurationException(
						"Document schema is not readable");
			}
	...
	public File getRuleSetsDir() throws ConfigurationException {
		...
		if (!schematron.isDirectory()) {
			schematron = new File(getBaseDir() + "/"
					+ configuration.getSchematron().getDirectory());
		}
		if (!schematron.isDirectory()) {
		...
		if (!schematron.canRead()) {
			throw new ConfigurationException(
					"Schematron directory is not readable");
		}
	...
	private void createRuleSetMaps() {
		...
		for (RuleSet ruleSet : createRuleSetList()) {
			...
			if (ruleSet.getTemplateId() != null) {
				if ((first = ruleSetOidMap
						.put(ruleSet.getTemplateId(), ruleSet)) != null) {
					Object[] args = new Object[] { ruleSet.getTemplateId(),
							first.getPath().getName(),
							ruleSet.getPath().getName() };
					log.error("RuleSet OID {} is not unique."
							+ " It was declared in '{}' and '{}'", args);
				}
			}
		}
	}
