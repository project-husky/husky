...
public class LaboratoryObservationTextBuilder extends TextBuilder {
	...
	private void addBody() {
		append("<tbody>");
		int i = 1;
		for (final org.ehealth_connector.cda.LaboratoryObservation problemConcern : laboratoryObservation) {
		...
		for (final EntryRelationship er : laboratoryObservation.getMdhtLaboratoryObservation()
			...
			if (Util.isComment(er)) {
				...
				cellStr = cellStr + ("<content ID='"
						+ SectionsVACD.SEROLOGY_STUDIES.getContentIdPrefix() + "-comment"
