...
public class Immunization extends MdhtFacade<org.openhealthtools.mdht.uml.cda.ch.Immunization> {
	 ...
	 */
	public Immunization() {
		super(ChFactory.eINSTANCE.createImmunization().init(), AbstractCdaChV1.OID_V1,
				"CDA-CH.Body.MediL3");
	...
	private SXCM_TS convertDate(Date appliedAt) {
		...
		timestamp.setValue(DateUtilMdht.formatDate(appliedAt));
		return timestamp;
	}
	...
	public void setCode(MedicationsSpecialConditions specialCode) {
		...
		getMdht().getEffectiveTimes().add(DateUtilMdht.createSTCM_TS(new Date()));
