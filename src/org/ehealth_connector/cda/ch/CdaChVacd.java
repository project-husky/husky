 ...
 */
public class CdaChVacd extends CdaCh<VACD> {
	...
	protected VACD correctSectionSequence() {
		...
		final VACD newVacd = EcoreUtil.copy(this.getMdht());
		...
		final VACD oldVacd = this.getMdht();
		...
		CdaUtil.addSectionToStructuredBodyAsCopy(sb, oldVacd.getLaboratorySpecialitySection());
		...
		for (final Act act : acts) {
			...
			for (final ProblemEntry problemEntry : problemConcernEntry.getAllergyIntolerances()) {
				...
				for (final EntryRelationship er : problemEntry.getEntryRelationships()) {
					j++;
					CdaUtil.updateRefIfComment(er, String.valueOf(i) + String.valueOf(j),
							loincSectionCode);
				}
			}
		}
			...
			for (int j = 0; j < lba.getLaboratoryObservations().size(); j++) {
				...
				for (int k = 0; k < lo.getEntryRelationships().size(); k++) {
					...
					if (Util.isComment(er)) {
						++k;
						CdaUtil.updateRefIfComment(er, String.valueOf(i + 1) + String.valueOf(j + 1)
