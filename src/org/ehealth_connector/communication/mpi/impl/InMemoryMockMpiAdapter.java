	...
	public boolean addPatient(FhirPatient patient) {
		...
		if (mapKey != null) {
			if (map.containsKey(mapKey)) {
				log.error("patient exists already with  "
			...
			log.info("added patient to mpi with key " + mapKey
					+ " and mpi idendity " + mpiCommunityOid + ","
					+ mpiIdendity);
			return true;
		} else {
			...
			if (patient.getIdentifier().size() == 1) {
				...
				if (system.length() > 8) {
					...
					throw new IllegalStateException(
							"system has to start with urn:oid:");
				}
			} else {
				throw new IllegalStateException(
						"homeCommunityId has to be specified");
			}
		}
	}
		...
		if (mapKey != null) {
			if (map.containsKey(mapKey)) {
				...
				for (int i = 0; i < returnIds.length; ++i) {
					returnIds[i] = getPatientId(patientMpi, "urn:oid:"
							+ domainOids[i]);
				}
				return returnIds;
			}
