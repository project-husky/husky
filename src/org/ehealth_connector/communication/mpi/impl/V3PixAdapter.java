 ...
 * V3PixAdapter
 * 
 * V3PixAdapter implements the Actor Patient Identity Source from ITI-44 Patient
...
public class V3PixAdapter implements MpiAdapterInterface {
	...
	public void setMultipeBirth(FhirPatient patient,
		...
		if (iMultipleBirth instanceof IntegerDt) {
			...
			if (multipleBirth.getValue() != null) {
				v3PixSourceMessage.setMultipleBirthOrderNumber(multipleBirth
						.getValue());
		...
		if (iMultipleBirth instanceof BooleanDt) {
			...
			if (multipleBirth.getValue() != null) {
				v3PixSourceMessage.setMultipleBirthIndicator(multipleBirth
						.getValue());
			}
		}
	}
	...
	/**
	 * Adds the marital status. TODO is the coding of marital status of fhir
	 * equivalent to HL7 V3?
	 * http://hl7.org/implement/standards/FHIR-Develop/valueset
	 * -marital-status.html
	 * 
	 * @param patient
	 *            the patient
